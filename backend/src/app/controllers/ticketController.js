...
router.get('/:ticket_id', [
    ...
    try {
        console.log('1');
        let result = await ticket.get(req);
        ...
        if (result.message == "ticket Doesnot exist") {
            ...
            <body >
            <h1><marquee scrollamount="20" behavior="alternate" direction="left">PLAY BINGO!!!</marquee></h1>
            ...
            <h3><a href="http://localhost:8002/api/game/` + result.data.game_id + `/stats">Current Game Stats</a></h3>
            </body>
            </html>`;
...
router.post('/:game_id/ticket/:username/generate/', async (req, res, next) => {

    try {
        console.log('1');
        let result = await ticket.createTicket(req);
        res.status(200).json(result);
